...
import { CreateRegisterPlan } from "../create-register-plan/create-register-plan";
...
import { RemovePlanUseCase } from "./remove-plan";
...
describe("create-register-plan", () => {
  ...
  let removePlan: RemovePlanUseCase;
  ...
  beforeAll(() => {
    ...
    removePlan = new RemovePlanUseCase(
      inMemoryUserRepository,
      inMemoryRegisterPlan
    );
  });

  test("a user must be able to remove a plan", async () => {
    const userData: User = {
      first_name: "Joe",
      surname: "Doe",
    ...
    await createRegisterPlan.execute(createRecordPlanData, 49990);

    await removePlan.execute({ user_id: user.id });

    const allPlans = await inMemoryRegisterPlan.getAll();
